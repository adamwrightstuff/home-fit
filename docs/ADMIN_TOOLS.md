...
```bash
./scripts/ops/check_backend_score.sh
# Or with a location:
./scripts/ops/check_backend_score.sh "Austin, TX"
```
...
PYTHONPATH=. python3 scripts/collectors/score_locations.py
# Save to file:
PYTHONPATH=. python3 scripts/collectors/score_locations.py > scores.csv
```
...
```bash
PYTHONPATH=. python3 scripts/debug/validate_economic_security.py --quick --delay 0.3
```

### API health check (local or set base_url)
Hits `/`, `/health`, `/score`; checks calibration. Default: localhost:8000.
```bash
python3 scripts/manual/test_api_health.py
# Or with Railway:
python3 scripts/manual/test_api_health.py --base-url https://home-fit-production.up.railway.app
```
...
```bash
PYTHONPATH=. python3 scripts/baselines/build_economic_baselines.py \
...
| Backend score check | `./scripts/ops/check_backend_score.sh` |
| Score 40 locations  | `scripts/collectors/score_locations.py` |
| Validate econ pillar| `scripts/debug/validate_economic_security.py` |
| API health check    | `scripts/manual/test_api_health.py` |
| Rebuild baselines   | `scripts/baselines/build_economic_baselines.py` |
