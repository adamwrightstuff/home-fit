  ...
  if (!showSearch && selected) {
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--hf-text-primary)', lineHeight: 1.3 }}>{selected.label}</div>
          <div style={{ fontSize: 12, color: 'var(--hf-text-secondary)', lineHeight: 1.35, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {snapped
        ...
        <button
          ...
          style={{
            width: 32, height: 32, borderRadius: 999, flexShrink: 0, border: 'none', cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
    ...
    <div ref={rootRef} style={{ position: 'relative', marginBottom: 10 }}>
      <div
        style={{
          ...
          boxShadow: focused ? '0 2px 10px rgba(0,0,0,0.10)' : '0 1px 2px rgba(0,0,0,0.04)',
          transition: 'box-shadow 150ms ease, border-color 150ms ease',
        }}
      >
        {busy
          ? <Loader2 size={18} className="animate-spin" style={{ color: 'var(--hf-primary-1)', flexShrink: 0 }} aria-hidden />
          : <Search size={18} strokeWidth={2.25} style={{ color: 'var(--hf-text-primary)', flexShrink: 0 }} aria-hidden />}
        <input
          ref={inputRef}
          type="text"
          role="combobox"
          ...
          style={{
            ...
            fontSize: 16, color: 'var(--hf-text-primary)', // 16px avoids iOS focus zoom
          }}
        />
        {editing && selected && (
          <button
            type="button"
            onClick={finish}
            style={{ fontSize: 12, fontWeight: 600, color: 'var(--hf-text-secondary)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
          >
            Cancel
          </button>
        )}
      ...
      {open && options.length > 0 && (
        <ul
          ...
          style={{
            ...
            margin: 0, padding: 8, listStyle: 'none',
            maxHeight: 300, overflowY: 'auto',
            borderRadius: 20, background: 'var(--hf-card-bg)',
            boxShadow: '0 8px 28px rgba(0,0,0,0.16)',
          }}
        >
          {!query.trim() && (
            <li aria-hidden style={{ padding: '6px 10px 4px', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--hf-text-tertiary)' }}>
              Job hubs
            </li>
          )}
          {options.map((opt, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isAddress ? <MapPin size={18} /> : <Building2 size={18} />}
                </div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, color: 'var(--hf-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {isAddress ? <>Use &ldquo;{opt.query}&rdquo;</> : highlight(opt.zone.label, query)}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--hf-text-secondary)' }}>
      ...
      {error && (
        <div role="alert" style={{ marginTop: 6, paddingLeft: 14, fontSize: 12, color: 'var(--hf-danger)' }}>{error}</div>
      )}
    </div>
  )
}
