...
export default function PillarTwinDrawer({ open, onClose, selected, onChange }: PillarTwinDrawerProps) {
  if (!open) return null
  ...
  const selectAll = () => onChange(new Set(PILLAR_ORDER))
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--hf-border)] px-4 py-3">
          <div>
            ...
            <p className="text-xs text-[var(--hf-text-secondary)]">
              {count} of {PILLAR_ORDER.length} pillars · Euclidean match across selected scores
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
          <button
            type="button"
            className="mb-2 text-xs font-semibold text-[var(--hf-primary-1)]"
            onClick={selectAll}
          >
            Select all
          </button>
          <ul className="space-y-1.5">
            {PILLAR_ORDER.map((k) => (
              <li key={k}>
                <label className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[var(--hf-hover-bg)]">
                  <input
                    type="checkbox"
