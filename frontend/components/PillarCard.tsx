...
const METRIC_EXPLAINERS: Record<string, string> = {
  ...
  'public_transit_access:Walk to nearest heavy rail': 'Distance to the nearest subway or commuter rail stop we found.',
  'public_transit_access:Heavy rail connectivity': 'How well that line connects to the wider region (tier label).',
  'public_transit_access:Typical commute (tract)': 'Census-reported mean commute time for this tract (minutes).',
  ...
  'economic_opportunity:Unemployment rate': 'Local unemployment rate for the comparison geography.',
  'economic_opportunity:Median earnings (area)': 'Typical annual earnings in the comparison area (dollars).',
  'economic_opportunity:Comparison area': 'Named place or region we compare job-market stats to.',
  'quality_education:Average school rating': 'SchoolDigger composite (stars)—typically on a ~0–5 scale.',
  'quality_education:Schools with ratings': 'How many nearby schools have usable rating data.',
  'quality_education:Top-tier schools nearby': 'Count of schools rated in the top tier for this run.',
  'housing_value:Median home value (tract)': 'Census tract median home value (dollars).',
...
export default function PillarCard({
  ...
  const builtMetrics = pillar_key === 'built_environment' ? pillar.details?.architectural_analysis?.metrics : null
  const summary =
    pillar_key === 'built_environment' && isRecord(builtMetrics)
      ? {
          ...rawSummary,
        ...
        <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--hf-border)' }}>
          ...
          <div className="tr-muted" style={{ fontSize: '0.95rem' }}>
            ...
            {pillar_key === 'built_environment' && (() => {
              ...
              return (
                <div style={{ marginBottom: canDens ? '0.85rem' : '0.5rem' }}>
                  <div>
