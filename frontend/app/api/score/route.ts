...
const SCORE_PROXY_TIMEOUT_MS_JOB_CATEGORIES = Number(
  process.env.HOMEFIT_SCORE_PROXY_TIMEOUT_MS_JOB_CATEGORIES || '55000'
);
const PREMIUM_CODES = (() => {
...
export async function GET(req: NextRequest) {
  ...
  for (const key of ['tokens', 'priorities', 'include_chains', 'diagnostics', 'only', 'job_categories', 'natural_beauty_preference', 'built_character_preference', 'built_density_preference', 'diversity_preference', 'lat', 'lon', 'mode', 'trip_type', 'travel_month'] as const) {
    const v = sp.get(key);
  ...
  try {
    ...
    // - 202 {job_id, status} when not done yet (so the browser doesn't log it as a console error)
